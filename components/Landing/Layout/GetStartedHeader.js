	...
	return (
		<>
			<header
				className={
					styles.header +
					" fixed top-0 w-full bg-white-500 z-30 transition pt-4 " +
					(scrollActive ? styles.stick : "")
				}
			>
				<nav className="max-w-screen-xl px-6 sm:px-8 lg:px-16 mx-auto flex flex-row justify-between py-3 sm:py-4">
					<div className="col-start-1 col-end-2 flex items-center">
						<Image src="/assets/logo.svg" alt="logo" height={36} width={140} />
					</div>
					<ul className="hidden lg:flex col-start-4 col-end-8 text-black-500 items-center">
						<ButtonOutlined onClick={() => router.push("/")} filled>
							Back to Home
						</ButtonOutlined>
						<LinkScroll
							activeClass="active"
					...
					<div className="col-start-10 col-end-12 font-medium flex justify-end items-center">
						{token === null ? (
							...
							<ButtonOutlined onClick={() => router.push(`/panel/${role}`)}>
								Dashboard
							</ButtonOutlined>
						)}
					</div>
				...
				<div className="bg-white-500 sm:px-3">
					<ul className="flex w-full justify-between items-center text-black-500">
						<div
							onClick={() => router.push("/")}
							className={
								"mx-1 sm:mx-2 px-3 sm:px-4 py-2 flex flex-col items-center text-xs border-t-2 transition-all border-orange-500 text-orange-500"
							}
						>
							<svg
								className="w-6 h-6"
